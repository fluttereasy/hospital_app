        ...
        BlocProvider(
          create: (context) => ProfileBloc()
            ..add(ProfileLoadingEvent(
                mobileNumber: OtpScreen.numberForProfileScreen)),
        ),
      ],
      child: Scaffold(
        key: _scaffoldKey,
        bottomNavigationBar: BlocListener<ProfileBloc, ProfileState>(
          listener: (context, state) {
            if (state is ProfileLoadedState) {
              final userData = state.details;

              // selecting patient details automatically if available
              userData.dataInfo!.patientName != null
                  ? nameController.text =
                      userData.dataInfo!.patientName.toString()
                  : nameController.text == '';
              //selecting email automatically
              userData.dataInfo!.email != null
                  ? emailController.text = userData.dataInfo!.email.toString()
                  : emailController.text = '';

              // selecting number automatically
              userData.dataInfo != null
                  ? phoneNumberController.text =
                      userData.dataInfo!.mobileNo.toString()
                  : null;
              // selecting gender automatically
              gender = userData.dataInfo!.gender;

              //if gender is male setting the radio value button to 1 which is  male
              if (gender == 'Male') {
                radioValueBUtton = 1;
              } else {
                //if gender is Female setting the radio value button to 2 which is  male
                if (gender == 'Female') {
                  radioValueBUtton = 2;
                }
              }
                        ...
                        if (_paymentOption == 'Pay at Hospital') {
                          ...
                          context.read<PatientDetailsBloc>().add(
                              SendPatientDetailstoOnlineAPpointment(
                                  ...
                                  emailID: emailController.text,
                                  charges:
                                      DoctorListScreen.chargesForSubmitting,
                                  chargesType:
                          ...
                          setState(() {
                            ...
                            PatientDetails.numberforOnlinePayment =
                                phoneNumberController.text;
                            PatientDetails.nameForOnlinePayment =
                                nameController.text;
                            PatientDetails.emailForOnlinePayment =
                                emailController.text;
